# Abbas

<div align="center">
<img src="https://raw.githubusercontent.com/B14cs/Abbas/master/Header/header.jpg" align="center" style="width: 100%" />
</div>

## Abbas Telegram Bot is a simple and easy-to-use conversational AI-assistant running on GPT-3.5 language models.

<p align="center">
<a href="https://t.me/Abbas_funBOT?start=source=github" alt="Run Telegram Bot shield"><img src="https://img.shields.io/badge/RUN-Telegram%20Bot-blue?logo=data:image/svg+xml;base64," width="230"/></a>
</p>

## Features of 1st version
* Responds to user inputs in text format using OpenAI [gpt-3.5-turbo](https://platform.openai.com/docs/models/gpt-3-5) Language Models.

## Features of 2nd version
* Responds to user inputs in text format using OpenAI [gpt-3.5-turbo](https://platform.openai.com/docs/models/gpt-3-5) Language Models.
* Voice recognition with OpenAI Whisper ASR to handle voice inputs.
* Reset the mechanism for clearing the conversation history.

## Requirements
* Python 3.x installed
* Install necessary Python packages using the requirementsV1.txt file for the first version or requirementsV2.txt file for the second.
  ```
  pip install -r requirementsV1.txt
  or
  pip install -r requirementsV2.txt
  ```
  
  ## Environment Variables
* `telegram_token`: Your Telegram Bot Token which can be obtained from [BotFather](https://t.me/BotFather).
* `api_key`: Your OpenAI API Key, which can be found on the [OpenAI Dashboard](https://platform.openai.com/account/api-keys).

## Usage
1. Set your environment variables.

2. Run the script.

3. Open the Telegram app and interact with the bot using text messages or voice inputs.
